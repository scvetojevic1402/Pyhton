...
def main():
    ...
    for db in databases:
        for table in tables:
            ...
            c0 = con2.cursor(cursor_factory=psycopg2.extras.DictCursor)#more convenient than a regular sursor since 
            c1 = con1.cursor(cursor_factory=psycopg2.extras.DictCursor)#columns can be reffered by names 
            c2 = con1.cursor(cursor_factory=psycopg2.extras.DictCursor)#and not only by indecies
            c3 = con2.cursor()
            id_load_start = time.time()
            if(table=='tweets_stream_ne_i'):region_id=1
            elif(table=='tweets_stream_ne_ii'):region_id=2
            elif(table=='tweets_stream_nw_i'):region_id=3
            elif(table=='tweets_stream_nw_ii'):region_id=4
            elif(table=='tweets_stream_nw_iii'):region_id=5
            elif(table=='tweets_stream_se'):region_id=6
            elif(table=='tweets_stream_sw'):region_id=7
            if(db=='twitter_geo_split'):db_index=0
            else:db_index=db[len(db)-1:]
            max_id=0
